...
def test_good_date(app, auth_header):  # parent_id:2 = John Jay
    resp = app.get("/day/2014-10-21/building/153", headers=auth_header)
...
def test_bad_date(app, auth_header):
    resp = app.get("/day/23/building/153", headers=auth_header)
    body = json.loads(resp.data.decode())
...
def test_no_auth(app):
    resp = app.get("/day/2014-10-21/building/153")
    body = json.loads(resp.data.decode())
